...
var F=false,T=true,X='FaceTimelapse',D,P,Z=0,L,H,G=PermissionGroup.values,W=Colors.white;
...
class HPS extends State<HP>{
  ...
  build(k)=>Scaffold(
      appBar:AppBar(
        title:Text(X),
        actions:[
          B(Icons.movie,()=>M(k,VP())),
          B(Icons.add_a_photo,()=>M(k,TP()).then((_)=>iI()))
        ]
      ),
      body:ScrollGallery(i.map((s)=>Image.file(s).image).toList().reversed.toList(),fit:BoxFit.cover,borderColor:W)
    );
}
